...
using the `ratter-build_container.df` docker image.
...
[source,bash]
----
rattler-build build --recipe "./packages/testresources" --target-platform linux-64 --channel mesomorph --channel conda-forge
----
