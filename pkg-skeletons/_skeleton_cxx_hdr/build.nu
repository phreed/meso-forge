...
def find-headers [] {
    ...
    for dir in $possible_dirs {
        if ($dir | path exists) {
            let headers = (ls $dir -r | where name =~ '\.(h|hpp|hxx|hh|H)$')
            if ($headers | length) > 0 {
...
def install-headers [source_dir: string, target_dir: string] {
    ...
    let headers = (ls $source_dir -r | where name =~ '\.(h|hpp|hxx|hh|H)$')
...
def generate-cmake-config [install_prefix: string, lib_name: string, version: string] {
    ...
    let config_content = $"
# ($lib_name) CMake Configuration File
...
def build-meson [] {
    ...
    let meson_args = [
        ...
        "--buildtype=release"
        $"-Dtests=($config.build_tests)"
        $"-Dexamples=($config.build_examples)"
    ]
...
def validate-installation [] {
    ...
    let installed_headers = (ls $include_base -r | where name =~ '\.(h|hpp|hxx|hh|H)$')
    ...
    let cmake_files = (ls ($config.install_prefix | path join "share") -r | where name =~ 'Config\.cmake$')
    if ($cmake_files | length) > 0 {
        log success $"Found ($cmake_files | length) CMake configuration files"
    }

    # Check for pkg-config files
    let pc_files = (ls ($config.install_prefix | path join "share") -r | where name =~ '\.pc$')
    ...
    log info $"  Total size: ((ls $include_base "-r" | where type == 'file' | get size | math sum) / 1KB | math round) KB"
}
