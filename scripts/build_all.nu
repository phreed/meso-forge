...
def main [
    ...
    let packages = ls $in_dir | where type == dir | get name
    ...
    build_noarch_packages --out-dir $out_dir

    # Build platform-specific packages
    print $"🔧 Building platform-specific packages for ($current_platform)..."
    build_platform_specific_packages --platform $current_platform --in-dir $in_dir
...
def build_platform_specific_packages [
    ...
    for package in $platform_packages {
        ...
        try {
            rattler-build build --recipe $recipe_path --target-platform $platform --output-dir $out_dir
        } catch {
